...
export async function getUserByEmail(email: string): Promise<(User & { company_name: string }) | null> {
	const result = await getPool().query<User & { company_name: string }>(
...
export async function getSessionByToken(token: string): Promise<(Session & User & { company_name: string }) | null> {
	const result = await getPool().query<Session & User & { company_name: string }>(
...
export async function deleteSession(token: string): Promise<void> {
	await getPool().query('DELETE FROM auth_sessions WHERE session_token = $1', [token]);
}
...
export async function getCompanyBySlug(slug: string): Promise<Company | null> {
	const result = await getPool().query<Company>(
...
export async function getVerificationToken(
	...
	const result = await getPool().query<{ identifier: string; token: string; expires_at: Date }>(
...
export async function getPasswordResetToken(
	...
	const result = await getPool().query<{ id: string; user_id: string; expires_at: Date; used_at: Date | null }>(
